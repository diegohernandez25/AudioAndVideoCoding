...
dct::dct(int height, int width, cv::Size block_size){
    ...
    height_blk = ceil((float) height/block_size.height);
    width_blk = ceil((float) width/block_size.width);


    height_padded = height_blk * block_size.height;
    width_padded = width_blk * block_size.width;
...
dct::dct(cv::Mat image, cv::Size block_size) : dct(image.rows, image.cols, block_size) {
    this->image = image;
